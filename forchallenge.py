...
for ip in ipAddress:
    ...
    if (ipAddress[ip] == ''): # check if IP Address is empty
        ...
        checkFirstIdx = 0 # use for verifying first index does not contain deliminator '.'
        errorMessage = '' # store error message if invalid
        for ipSegment in ipAddress[ip]: # get character value
            ...
            if (ipSegment == '.' and checkFirstIdx >= 1): # check first indexl; does it contain an invalid start deliminator
                ...
                errorMessage = '{0} IP address is invalid'.format(ipAddress[ip])  # assign error message
                break # break loop, IP has an invalid start deliminator

            checkFirstIdx += 1

        if (errorMessage != ''): # if message is not empty, then print error
            print(errorMessage)
        else:
